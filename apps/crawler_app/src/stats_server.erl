...
handle_call(write_stats, From, {PageTotal,BinaryTotal,WordsTotal,File}) ->
	...
	io:format(File,"~w-~w-~w ~w:~w:~w Page_total: ~w Binary_total ~w Words_processed_total ~w Indexes ~w Words_coll ~w Index_coll ~w Memory ~w\n",[Year,Month,Day,Hour,Minute,Seconds,PageTotal,BinaryTotal,WordsTotal, Indexes, Words_coll,Index_coll,Memory]),
