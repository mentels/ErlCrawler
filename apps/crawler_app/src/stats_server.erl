...
-export([add_page_stats/2,write_stats/0,get_stats/0]).
...
handle_call(write_stats, From, {PageTotal,BinaryTotal,WordsTotal,File}) ->
	...
	io:format(File,"~w-~w-~w ~w:~w:~w ~w~w~w Page_total: ~w Binary_total ~w Words_processed_total ~w Memory ~w\n",[Year,Month,Day,Hour,Minute,Seconds,Mega_sec,Sec,Milisec,PageTotal,BinaryTotal,WordsTotal,Memory]),
